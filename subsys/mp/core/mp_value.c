...
static int mp_value_set_allocted(mp_value_t *value)
{
	...
	if (MP_VALUE_IS_IMMEDIATE(*value)) {
		*value = k_calloc(1, mp_value_type_sizes[MP_VALUE_GET_TYPE(*value)]);
		if (*value == NULL) {
...
static int mp_value_set_range(mp_value_t *value, int type, va_list *args)
{
	...
	int64_t step = va_arg(*args, int64_t);

	if (mp_value_range_is_fitting(min, max, step)) {
		*value = MP_VALUE_NEW_IMMEDIATE(MP_TYPE_RANGE, MP_VALUE_PACK_RANGE(min, max, step));
		return 0;
	}

	ret = mp_value_set_allocted(value);
	if (ret < 0) {
		return ret;
	}
...
mp_value_t mp_value_new_empty(enum mp_value_type type)
{
	...
	case MP_TYPE_RANGE:
		return (mp_value_t)MP_VALUE_NEW_IMMEDIATE(
			MP_TYPE_RANGE, MP_VALUE_PACK_RANGE(0, 0, 0));
	case MP_TYPE_OBJECT:
	...
	if (value == NULL) {
		LOG_ERR("Failed to allocate mp_value_t of type %d", type);
		return NULL;
	}
	...
	if (mp_value_get_type(compare_val) == MP_TYPE_RANGE &&
		...
		intersect_value = MP_VALUE_NEW_INTERSECT_RANGE(ref_val, compare_val);
		   ...
		   IN_RANGE(mp_value_get_int(compare_val),                                                                   \
			    mp_value_get_range_min(ref_val), mp_value_get_range_max(ref_val))) {
...
{
	printk("[%lld, %lld, %lld]", mp_value_get_range_min(value),
	       mp_value_get_range_max(value), mp_value_get_range_step(value));
}
