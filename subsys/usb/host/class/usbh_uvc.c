...
struct uvc_device_descriptors {
	...
	const struct uvc_input_terminal_descriptor *vc_input;
	const struct uvc_output_terminal_descriptor *vc_output;
	const struct uvc_camera_terminal_descriptor *vc_camera;
	const struct uvc_selector_unit_descriptor *vc_selector;
	const struct uvc_processing_unit_descriptor *vc_processing;
	const struct uvc_encoding_unit_descriptor *vc_encoding;
	const struct uvc_extension_unit_descriptor *vc_extension;
...
struct uvc_format_info {
	...
	uint8_t frame_index;
	const struct uvc_format_common_descriptor *format_ptr;
...
struct uvc_host_data {
	...
	const struct usb_if_descriptor
		*stream_iface_alts[CONFIG_USBH_VIDEO_MAX_STREAM_INTERFACE_ALT];
	struct uvc_stream_iface_info current_stream_iface_info;
	struct uvc_device_descriptors uvc_descriptors;

	const struct uvc_format_mjpeg_descriptor *mjpeg_format[CONFIG_USBH_VIDEO_MAX_FORMATS];
...
static int configure_device(struct usbh_class_data *const c_data)
{
	...
	LOG_INF("Device configured successfully: control interface %u, streaming interface %u",
		ctrl_iface->bInterfaceNumber, stream_iface->bInterfaceNumber);

	return 0;
}

static bool vs_header_is_valid(const void *const desc)
{
...
static bool vc_header_is_valid(const void *const desc)
{
...
static const void *get_vs_desc_end(const struct usb_if_descriptor *const if_desc)
{
	...
	total_length = sys_le16_to_cpu(header_desc->wTotalLength);
	...
	for (; desc != NULL && (const uint8_t *)desc < (const uint8_t *)desc_end;
		...
		case UVC_VC_INPUT_TERMINAL: {
			...
			if (sys_le16_to_cpu(it_desc->wTerminalType) == UVC_ITT_CAMERA) {
				host_data->uvc_descriptors.vc_camera = ct_desc;
				LOG_DBG("Found Camera Terminal: ID=%u", ct_desc->bTerminalID);
			} else {
				host_data->uvc_descriptors.vc_input = it_desc;
			 ...
			 const void *const desc_end)
{
	...
	for (; desc != NULL && (const uint8_t *)desc < (const uint8_t *)desc_end;
		...
		case UVC_VS_FORMAT_UNCOMPRESSED: {
			...
			host_data->uncompressed_format[host_data->num_uncompressed_formats] =
				format_desc;
			host_data->num_uncompressed_formats++;
			LOG_DBG("Added uncompressed format[%u]: index=%u",
				host_data->num_uncompressed_formats - 1, format_desc->bFormatIndex);
			break;
		}
		...
		case UVC_VS_FORMAT_MJPEG: {
			...
			host_data->mjpeg_format[host_data->num_mjpeg_formats] = format_desc;
			host_data->num_mjpeg_formats++;
			LOG_DBG("Added MJPEG format[%u]: index=%u",
				host_data->num_mjpeg_formats - 1, format_desc->bFormatIndex);
			break;
		}
...
static void parse_vs_interface_alt_desc(struct uvc_host_data *const host_data,
					const uint8_t iface)
{
	...
	for (; if_desc != NULL; if_desc = (const void *)usbh_desc_get_next_alt_setting(if_desc)) {
		...
		for (i = 0; i < CONFIG_USBH_VIDEO_MAX_STREAM_INTERFACE_ALT; i++) {
			if (host_data->stream_iface_alts[i] != NULL) {
				continue;
			}

			host_data->stream_iface_alts[i] = if_desc;
			LOG_DBG("Added VideoStreaming interface %u alt %u to stream_iface_alts[%d]",
				if_desc->bInterfaceNumber, if_desc->bAlternateSetting, i);
			break;
		}

		if (i == CONFIG_USBH_VIDEO_MAX_STREAM_INTERFACE_ALT) {
			LOG_WRN("Out of space, consider raising %s above %u",
				"CONFIG_USBH_VIDEO_MAX_STREAM_INTERFACE_ALT",
				CONFIG_USBH_VIDEO_MAX_STREAM_INTERFACE_ALT);
		}
	}
}
...
static int parse_descriptors(struct usbh_class_data *const c_data, uint8_t iface)
{
	...
	for (uint8_t i = 0; i < iad_desc->bInterfaceCount; i++) {
		...
		if (if_desc->bInterfaceClass == USB_BCC_VIDEO &&
			...
			parse_vs_interface_alt_desc(host_data, if_desc->bInterfaceNumber);
		}
	}
...
static uint32_t get_frame_default_interval(const struct uvc_frame_common_descriptor *frame_desc)
{
	...
	if (frame_desc->bDescriptorSubtype == UVC_VS_FRAME_FRAME_BASED) {
		const struct uvc_frame_framebased_continuous_descriptor *fb_cont_desc =
			(const void *)frame_desc;

		default_interval = sys_le32_to_cpu(fb_cont_desc->dwDefaultFrameInterval);
		interval_type = fb_cont_desc->bFrameIntervalType;

		if (interval_type == 0) {
			max_interval = sys_le32_to_cpu(fb_cont_desc->dwMaxFrameInterval);
		} else {
			const struct uvc_frame_framebased_discrete_descriptor *fb_disc_desc =
				(const void *)frame_desc;
			max_interval = sys_le32_to_cpu(
				fb_disc_desc->dwFrameInterval[interval_type - 1]);
		}
	}

	if (frame_desc->bDescriptorSubtype == UVC_VS_FRAME_UNCOMPRESSED ||
	    frame_desc->bDescriptorSubtype == UVC_VS_FRAME_MJPEG) {
		const struct uvc_frame_uncomp_continuous_descriptor *std_cont_desc =
			(const void *)frame_desc;

		default_interval = sys_le32_to_cpu(std_cont_desc->dwDefaultFrameInterval);
		interval_type = std_cont_desc->bFrameIntervalType;

		if (interval_type == 0) {
			max_interval = sys_le32_to_cpu(std_cont_desc->dwMaxFrameInterval);
		} else {
			const struct uvc_frame_uncomp_discrete_descriptor *std_disc_desc =
				(const void *)frame_desc;
			max_interval = sys_le32_to_cpu(
				std_disc_desc->dwFrameInterval[interval_type - 1]);
		}
	}
	...
	if (interval_type >= CONFIG_USBH_VIDEO_MAX_FRMIVAL) {
		LOG_WRN("Frame interval type %u exceeds maximum %u, using last valid index",
			interval_type, CONFIG_USBH_VIDEO_MAX_FRMIVAL);
	}
...
static int find_frame_in_format(const struct uvc_format_common_descriptor *format_header,
	...
	for (int i = 0; i < format_header->bNumFrameDescriptors;) {
		...
		if (frame_header->bDescriptorType == USB_DESC_CS_INTERFACE &&
			...
			if (frame_width == target_width && frame_height == target_height) {
				...
				*found_interval =
					get_frame_default_interval(frame_header);
...
static int find_format(struct uvc_host_data *const host_data,
		       const struct video_format *const fmt,
		       const struct uvc_format_common_descriptor **const format,
		       const struct uvc_frame_common_descriptor **const frame,
		       uint32_t *const frmival)
{
	...
	for (uint8_t i = 0; i < host_data->num_uncompressed_formats; i++) {
		uncomp_format = host_data->uncompressed_format[i];
		if (uncomp_format == NULL) {
			continue;
		}

		desc_pixelformat = uvc_guid_to_fourcc(uncomp_format->guidFormat);
		if (desc_pixelformat == fmt->pixelformat) {
			LOG_DBG("Found matching uncompressed format: index=%u",
				uncomp_format->bFormatIndex);
	...
	if (fmt->pixelformat == VIDEO_PIX_FMT_JPEG) {
		for (uint8_t i = 0; i < host_data->num_mjpeg_formats; i++) {
			mjpeg_format = host_data->mjpeg_format[i];
			if (mjpeg_format == NULL) {
				continue;
			}

			LOG_DBG("Checking MJPEG format: index=%u", mjpeg_format->bFormatIndex);
...
static int vs_get(struct uvc_host_data *const host_data, const uint8_t request,
...
static int vs_set(struct uvc_host_data *const host_data, const uint8_t request,
...
static int vs_request(struct uvc_host_data *const host_data, uint8_t request,
		      uint8_t control_selector, void *data, uint8_t data_len)
{
	switch (request) {
	/* SET requests */
	case UVC_SET_CUR:
		return vs_set(host_data, request, control_selector, data, data_len);
...
static int set_format(struct uvc_host_data *const host_data,
		      struct video_format *const fmt)
{
	...
	host_data->probe.bFormatIndex = format->bFormatIndex;
	host_data->probe.bFrameIndex = frame->bFrameIndex;
	host_data->probe.dwFrameInterval = sys_cpu_to_le32(frmival);
	...
	stream_iface = stream_info->iface;
...
static int usbh_uvc_set_frmival(const struct device *dev, struct video_frmival *const frmival)
{
	...
	stream_iface = host_data->current_stream_iface_info.iface;
			       ...
			       struct video_format_cap *caps_array, int start_index)
{
	...
	while (head != NULL && frames_found < num_frames) {
		...
		if (head->bDescriptorType == USB_DESC_CS_INTERFACE &&
			...
			if (head->bLength >= sizeof(struct uvc_frame_common_descriptor)) {
				if (cap_index >= CONFIG_USBH_VIDEO_MAX_CAPS - 1) {
					LOG_WRN("Format caps array full at %d entries, stopping",
						cap_index);
					break;
				}

				width = sys_le16_to_cpu(frame_header->wWidth);
				height = sys_le16_to_cpu(frame_header->wHeight);
...
static int create_format_caps(struct uvc_host_data *const host_data)
{
	const struct uvc_format_uncomp_descriptor *uncomp_format;
	const struct uvc_format_mjpeg_descriptor *mjpeg_format;
...
static int get_device_caps(struct uvc_host_data *const host_data,
...
static int enum_frame_intervals(const struct uvc_frame_common_descriptor *frame_ptr,
				struct video_frmival_enum *frmival_enum)
{
	const struct uvc_frame_framebased_continuous_descriptor *fb_desc;
	const struct uvc_frame_uncomp_discrete_descriptor *discrete_desc;
	...
	if (frame_ptr->bDescriptorSubtype == UVC_VS_FRAME_FRAME_BASED) {
		fb_desc = (const void *)frame_ptr;

		interval_type = fb_desc->bFrameIntervalType;
		interval_data = ((const uint8_t *)frame_ptr) +
				offsetof(struct uvc_frame_framebased_continuous_descriptor,
					 bFrameIntervalType) +
				1;
	} else if (frame_ptr->bDescriptorSubtype == UVC_VS_FRAME_UNCOMPRESSED ||
		   frame_ptr->bDescriptorSubtype == UVC_VS_FRAME_MJPEG) {
		/* For both uncompressed and MJPEG, the structure layout is the same */
		discrete_desc = (const void *)frame_ptr;

		interval_type = discrete_desc->bFrameIntervalType;
...
static int vc_get(struct uvc_host_data *const host_data, const uint8_t request,
...
static int vc_set(struct uvc_host_data *const host_data, const uint8_t request,
		  const uint8_t control_selector, const uint8_t entity_id,
...
static bool pu_supports_control(struct uvc_host_data *const host_data,
				uint32_t bmcontrol_bit)
{
	const struct uvc_processing_unit_descriptor *pud =
		host_data->uvc_descriptors.vc_processing;
	uint32_t controls = 0;
...
static bool ct_supports_control(struct uvc_host_data *const host_data,
				uint32_t bmcontrol_bit)
{
	const struct uvc_camera_terminal_descriptor *vc_camera =
		host_data->uvc_descriptors.vc_camera;
	uint32_t controls = 0;
	...
	if (vc_camera->bControlSize == 0) {
		return false;
	}

	for (uint8_t i = 0; i < vc_camera->bControlSize && i < 3; i++) {
		controls |= ((uint32_t)vc_camera->bmControls[i]) << (i * 8);
	}
...
static int init_control_by_cid(const struct device *dev, uint32_t cid,
			       const struct video_ctrl_range *range)
{
	...
	case VIDEO_CID_AUTO_WHITE_BALANCE:
		ret = video_init_ctrl(&ctrls->auto_white_balance, dev, cid, *range);
		break;
...
static uint32_t extract_control_bitmap(const uint8_t *bmControls, uint8_t control_size)
{
	uint32_t controls = 0;
...
static int32_t extract_control_value(const uint8_t *data, uint8_t size, uint8_t type)
{
	switch (size) {
	case 1:
...
static int query_control_range(struct uvc_host_data *const host_data, uint8_t entity_id,
			       ...
			       struct video_ctrl_range *range)
{
	...
	/* Query minimum value */
	ret = vc_get(host_data, UVC_GET_MIN, map->selector, entity_id, data, map->size);
	if (ret != 0) {
		LOG_DBG("Failed to get min value for selector 0x%02x: %d", map->selector, ret);
		return ret;
	}
	range->min = extract_control_value(data, map->size, map->type);

	/* Query maximum value */
	ret = vc_get(host_data, UVC_GET_MAX, map->selector, entity_id, data, map->size);
	if (ret == -EPIPE) {
	...
	range->max = extract_control_value(data, map->size, map->type);

	/* Query step/resolution value */
	ret = vc_get(host_data, UVC_GET_RES, map->selector, entity_id, data, map->size);
	if (ret == -EPIPE) {
		LOG_WRN("Request 0x%02x not supported by device for entity %d, control selector "
			"0x%02x",
		...
		int32_t step_val = extract_control_value(data, map->size, map->type);

		range->step = (step_val > 0) ? step_val : 1;
	}

	/* Query default value */
	ret = vc_get(host_data, UVC_GET_DEF, map->selector, entity_id, data, map->size);
	if (ret == -EPIPE) {
		LOG_WRN("Request 0x%02x not supported by device for entity %d, control selector "
			"0x%02x",
		...
		range->def = extract_control_value(data, map->size, map->type);
	}
...
static int init_unit_controls(struct uvc_host_data *const host_data,
			      ...
			      int *initialized_count)
{
	...
	for (size_t i = 0; i < map_length; i++) {
		if ((supported_controls & BIT(map[i].bit)) == 0) {
			LOG_DBG("Control bit %u not supported by device", map[i].bit);
			continue;
		}

		ret = query_control_range(host_data, entity_id, &map[i], &range);
		if (ret != 0) {
			continue;
		}

		ret = init_control_by_cid(dev, map[i].cid, &range);
		if (ret == -ENOTSUP) {
			LOG_WRN("Control cid %u not supported by device", map[i].cid);
		} else if (ret != 0) {
			LOG_WRN("Failed to initialize control CID 0x%08x: %d", map[i].cid, ret);
		} else {
			(*initialized_count)++;
			LOG_DBG("Successfully initialized control CID 0x%08x", map[i].cid);
		}
	}
...
static int camera_init_controls(const struct device *dev)
{
	...
	if (desc->vc_processing == NULL) {
		LOG_WRN("No processing unit found, skipping control initialization");
		return 0;
	}

	LOG_DBG("Found Processing Unit (ID: %u)", desc->vc_processing->bUnitID);

	control_bitmap = extract_control_bitmap(desc->vc_processing->bmControls,
						desc->vc_processing->bControlSize);

	ret = init_unit_controls(host_data, dev, UVC_VC_PROCESSING_UNIT,
	...
	if (desc->vc_camera != NULL) {
		LOG_DBG("Found Camera Terminal (ID: %u)", desc->vc_camera->bTerminalID);

		control_bitmap = extract_control_bitmap(desc->vc_camera->bmControls,
							desc->vc_camera->bControlSize);

		ret = init_unit_controls(host_data, dev, UVC_VC_INPUT_TERMINAL,
	...
	if (desc->vc_selector != NULL) {
		LOG_DBG("Found Selector Unit (ID: %u)", desc->vc_selector->bUnitID);

		control_bitmap = BIT(0);

		ret = init_unit_controls(host_data, dev, UVC_VC_SELECTOR_UNIT,
	...
	if (desc->vc_extension != NULL) {
		LOG_DBG("Found Extension Unit (ID: %u)", desc->vc_extension->bUnitID);

		control_bitmap = extract_control_bitmap(
			desc->vc_extension->bmControls, desc->vc_extension->bControlSize);

		ret = init_unit_controls(host_data, dev, UVC_VC_EXTENSION_UNIT,
...
static int usbh_uvc_removed(struct usbh_class_data *const c_data)
{
	...
	memset(host_data->stream_iface_alts, 0, sizeof(host_data->stream_iface_alts));
	host_data->current_ctrl_iface = NULL;
	memset(&host_data->current_stream_iface_info, 0,
	       sizeof(host_data->current_stream_iface_info));

	LOG_DBG("Clearing UVC descriptors");
	memset(&host_data->uvc_descriptors, 0x00, sizeof(host_data->uvc_descriptors));
	memset(host_data->mjpeg_format, 0, sizeof(host_data->mjpeg_format));
	host_data->num_mjpeg_formats = 0;
	memset(host_data->uncompressed_format, 0, sizeof(host_data->uncompressed_format));
	host_data->num_uncompressed_formats = 0;
...
static int usbh_uvc_enum_frmival(const struct device *dev, struct video_frmival_enum *frmival_enum)
{
	...
	ret = enum_frame_intervals(host_data->current_format.frame_ptr, frmival_enum);
	if (ret != 0) {
...
static int search_control_in_entity(uint8_t subtype, uint32_t cid, uint8_t *found_type,
					     const struct uvc_control_map **map)
{
...
static int find_control_mapping(struct uvc_host_data *const host_data, uint32_t cid,
				...
				const struct uvc_control_map **map)
{
	...
	if (host_data->uvc_descriptors.vc_processing != NULL) {
		ret = search_control_in_entity(UVC_VC_PROCESSING_UNIT, cid, entity_subtype, map);
		if (ret == 0) {
			goto exit_find;
		}
	...
	if (host_data->uvc_descriptors.vc_camera != NULL) {
		ret = search_control_in_entity(UVC_VC_INPUT_TERMINAL, cid, entity_subtype, map);
		if (ret == 0) {
			goto exit_find;
		}
	...
	if (host_data->uvc_descriptors.vc_selector != NULL) {
		ret = search_control_in_entity(UVC_VC_SELECTOR_UNIT, cid, entity_subtype, map);
		if (ret == 0) {
			goto exit_find;
		}
	...
	if (host_data->uvc_descriptors.vc_extension != NULL) {
		ret = search_control_in_entity(UVC_VC_EXTENSION_UNIT, cid, entity_subtype, map);
		if (ret == 0) {
			goto exit_find;
		}
...
static uint8_t get_entity_id(struct uvc_host_data *const host_data, uint8_t unit_subtype)
{
	struct uvc_device_descriptors *desc = &host_data->uvc_descriptors;

	switch (unit_subtype) {
	case UVC_VC_PROCESSING_UNIT:
		return desc->vc_processing ? desc->vc_processing->bUnitID : 0;

	case UVC_VC_INPUT_TERMINAL:
		return desc->vc_camera ? desc->vc_camera->bTerminalID : 0;

	case UVC_VC_SELECTOR_UNIT:
		return desc->vc_selector ? desc->vc_selector->bUnitID : 0;
...
static int encode_control_value(struct video_control *ctrl,
...
static bool control_is_supported(struct uvc_host_data *const host_data,
				 uint8_t unit_subtype, uint8_t entity_id,
				 uint32_t control_bit)
{
	switch (unit_subtype) {
	case UVC_VC_PROCESSING_UNIT:
		return pu_supports_control(host_data, control_bit);

	case UVC_VC_INPUT_TERMINAL:
		return ct_supports_control(host_data, control_bit);
...
static int usbh_uvc_set_ctrl(const struct device *dev, uint32_t id)
{
	...
	if (!control_is_supported(host_data, unit_subtype, entity_id, BIT(map->bit))) {
...
static int get_control_value(struct uvc_host_data *const host_data, uint32_t cid, int32_t *value)
{
	const struct uvc_control_map *map;
	uint8_t data[UVC_MAX_CTRL_SIZE] = {0};
	uint8_t unit_subtype;
	...
	ret = find_control_mapping(host_data, cid, &unit_subtype, &map);
	if (ret != 0) {
...
static int usbh_uvc_get_volatile_ctrl(const struct device *dev, uint32_t id)
{
	...
	case VIDEO_CID_EXPOSURE_AUTO:
		ret = get_control_value(host_data, VIDEO_CID_EXPOSURE_AUTO,
	...
	case VIDEO_CID_AUTO_WHITE_BALANCE:
		ret = get_control_value(host_data, VIDEO_CID_AUTO_WHITE_BALANCE,
					&ctrls->auto_white_balance.val);
		if (ret != 0) {
	...
	case VIDEO_CID_EXPOSURE_ABSOLUTE:
		ret = get_control_value(host_data, VIDEO_CID_EXPOSURE_ABSOLUTE,
	...
	case VIDEO_CID_WHITE_BALANCE_TEMPERATURE:
		ret = get_control_value(host_data, VIDEO_CID_WHITE_BALANCE_TEMPERATURE,
...
static struct usbh_class_api usbh_uvc_class_api = {
	.init = usbh_uvc_init,
...
LISTIFY(CONFIG_USBH_VIDEO_INSTANCES_COUNT, USBH_VIDEO_DEVICE_DEFINE, (;), _)
