...
static bool usbh_uvc_desc_is_valid_vs_header(const void *const desc)
{
	...
	return usbh_desc_is_valid(desc, sizeof(struct uvc_stream_header_descriptor),
...
static bool usbh_uvc_desc_is_valid_vc_header(const void *const desc)
{
	...
	return usbh_desc_is_valid(desc, sizeof(struct uvc_control_header_descriptor),
				  USB_DESC_CS_INTERFACE) &&
...
static const void *usbh_uvc_desc_get_vs_end(const struct usb_if_descriptor *if_desc)
{
	const struct uvc_stream_header_descriptor *const header_desc =
		usbh_desc_get_next(if_desc);

	if (!usbh_uvc_desc_is_valid_vs_header(header_desc)) {
		return NULL;
	}

	return (uint8_t *)header_desc + sys_le16_to_cpu(header_desc->wTotalLength);
}
...
static int usbh_uvc_parse_vs_desc(struct uvc_device *uvc_dev, const void *const desc_beg)
{
	const struct usb_desc_header *desc = usbh_desc_get_next(desc_beg);
				      ...
				      struct uvc_device *uvc_dev, uint8_t iface)
{
	const struct usb_association_descriptor *const iad_desc =
		usbh_desc_get_iad(uvc_dev->udev, iface);
	const struct usb_if_descriptor *if_desc;
	...
	desc = (struct usb_desc_header *)usbh_desc_get_next(iad_desc);
	if (desc == NULL) {
		return -EBADMSG;
	}

	for (int i = 0; i < iad_desc->bInterfaceCount; i++) {
		if_desc = (const void *)usbh_desc_get_iface(uvc_dev->udev, c_data->iface + i);
		if (if_desc == NULL) {
		...
		if (if_desc->bInterfaceClass == USB_BCC_VIDEO &&
			...
			ret = usbh_uvc_parse_vs_desc(uvc_dev, if_desc);
			if (ret != 0) {
				return ret;
			}
 ...
 */
int uvc_host_preinit(const struct device *dev)
{
