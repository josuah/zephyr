...
static const struct video_reg8 default_mipi_csi_regs[] = {
	...
	{GC2145_REG_DPHY_MODE1, GC2145_DPHY_MODE1_CLK_EN |
				GC2145_DPHY_MODE1_LANE0_EN |
				GC2145_DPHY_MODE1_LANE1_EN |
...
enum {
	GC2145_CSI_LINK_FREQ_120MHZ,
	GC2145_CSI_LINK_FREQ_240MHZ,
};
const int64_t gc2145_csi_link_freq[] = {
	[GC2145_CSI_LINK_FREQ_120MHZ] = MHZ(120),
	[GC2145_CSI_LINK_FREQ_240MHZ] = MHZ(240),
};
			     ...
			     uint32_t width, uint32_t height)
{
	...
	case RESOLUTION_VGA_W:
		ctrls->link_freq.val = GC2145_CSI_LINK_FREQ_120MHZ;
		break;
	case RESOLUTION_UXGA_W:
		ctrls->link_freq.val = GC2145_CSI_LINK_FREQ_240MHZ;
		break;
	default:
...
enum {
	GC2145_DVP_LINK_FREQ_140MHZ,
};
const int64_t gc2145_dvp_link_freq[] = {
	[GC2145_DVP_LINK_FREQ_140MHZ] = MHZ(140),
};
static int gc2145_config_dvp(const struct device *dev, uint32_t pixelformat,
			     uint32_t width, uint32_t height)
{
	struct gc2145_data *drv_data = dev->data;
	struct gc2145_ctrls *ctrls = &drv_data->ctrls;

	ctrls->link_freq.val = GC2145_DVP_LINK_FREQ_140MHZ;
...
static int gc2145_set_fmt(const struct device *dev, struct video_format *fmt)
{
	...
	if (cfg->bus_type == VIDEO_BUS_TYPE_CSI2_DPHY) {
		...
		ret = gc2145_config_dvp(dev, fmt->pixelformat, fmt->width, fmt->height);
		if (ret < 0) {
			LOG_ERR("Failed to configure DVP");
			return ret;
		}
	}
...
static int gc2145_init_controls(const struct device *dev)
{
	...
	if (cfg->bus_type == VIDEO_BUS_TYPE_CSI2_DPHY) {
		ret = video_init_int_menu_ctrl(
			&ctrls->link_freq, dev, VIDEO_CID_LINK_FREQ, 0,
			gc2145_csi_link_freq, ARRAY_SIZE(gc2145_csi_link_freq));
	} else {
		ret = video_init_int_menu_ctrl(
			&ctrls->link_freq, dev, VIDEO_CID_LINK_FREQ, 0,
			gc2145_dvp_link_freq, ARRAY_SIZE(gc2145_dvp_link_freq));
	}
	if (ret < 0) {
		return ret;
	}
