...
static inline void dwc2_set_reset(struct usb_dwc2_reg *const dwc2, const bool reset_on)
{
	uint32_t hprt;
...
static inline void dwc2_set_power(struct usb_dwc2_reg *const dwc2, const bool power_on)
{
	uint32_t hprt;
...
static int dwc2_flush_rx_fifo(struct usb_dwc2_reg *const dwc2)
{
...
static int dwc2_flush_tx_fifo(struct usb_dwc2_reg *const dwc2, const uint8_t fnum)
{
	const k_timepoint_t timepoint = sys_timepoint_calc(K_MSEC(100));
	uint32_t grstctl;
...
static void dwc2_config_timings(struct usb_dwc2_reg *const dwc2)
{
...
static inline int dwc2_init_gusbcfg(struct usb_dwc2_reg *const dwc2)
{
	uint32_t gusbcfg = sys_read32((mem_addr_t)&dwc2->gusbcfg);
	uint32_t ghwcfg2 = sys_read32((mem_addr_t)&dwc2->ghwcfg2);
...
static inline int dwc2_init_gahbcfg(struct usb_dwc2_reg *const dwc2)
{
	uint32_t ghwcfg2;
	uint32_t gahbcfg;
...
static int dwc2_set_fifo_sizes(struct usb_dwc2_reg *const dwc2)
{
	...
	ret = dwc2_flush_tx_fifo(dwc2, MAX_CHANNELS);
	if (ret != 0) {
		return ret;
	}

	return dwc2_flush_rx_fifo(dwc2);
}
...
static int uhc_dwc2_init(const struct device *const dev)
{
	...
	ret = dwc2_init_gusbcfg(dwc2);
	if (ret != 0) {
		LOG_ERR("Unable to configure USB global register");
		return ret;
	}

	ret = dwc2_init_gahbcfg(dwc2);
	if (ret != 0) {
		/* TODO: Implement Slave Mode */
