...
static uint32_t uhc_dwc2_channel_handle_irq_events(struct uhc_dwc2_channel *channel)
{
	...
	/*
	 * Completeion bits might come earlier, than channel halted interrupt.
	 * Save the hcint as pending and wait for channel halted.
	 */
	channel->hcint_pending |= hcint & USB_DWC2_HCINT_CPLT_BITS;

	if (hcint & USB_DWC2_HCINT_CHHLTD) {
		/* Channel halted */
		hcint |= channel->hcint_pending;
		channel->hcint_pending = 0U;
	} else if (hcint & USB_DWC2_HCINT_CPLT_BITS) {
		/* Channel not halted, wait for channel to be halted */
		return 0;
	}
