					  ...
					  const uint32_t len)
{
	...
	LOG_DBG("Starting DMA read, FIFO %u, data %p, len %u", fifo_idx, (void *)data, len);
					   ...
					   const uint32_t len)
{
	...
	LOG_DBG("Starting DMA write, FIFO %u, buf %p, len %u", fifo_idx, (void *)data, len);
...
static uint32_t udc_bflb_bl61x_ctrl_remain(const struct device *const dev)
{
	...
	tmp = (sys_read32(cfg->base + USB_VDMA_CXFPS1_OFFSET) & USB_VDMA_LEN_MASK);
					    ...
					    struct udc_bflb_bl61x_fifo *fifo)
{
	...
	LOG_INF("Buffer complete %u|%u|%u, ep 0x%02x, FIFO %u",
				     ...
				     struct net_buf *buf)
{
	...
	LOG_DBG("enqueue %p for ep 0x%02x, %u|%u|%u, size %u, len %u, fifo %p %u for 0x%02x, q sz %u",
		(void *)buf, ep_cfg->addr, bi->setup, bi->data, bi->status, buf->size, buf->len,
		(void *)fifo, fifo->idx, fifo->ep_cfg->addr, sys_sflist_len(&ep_cfg->fifo._queue.data_q));
